...
PKG_SUBDIRS=$(shell go list ./... | grep -v /vendor/ | sed -r 's|github.com/elxirhealth/key/||g' | sort)
...
acceptance:
	...
	@mkdir -p artifacts
	@go test -tags acceptance -v github.com/elxirhealth/key/pkg/acceptance 2>&1 | tee artifacts/acceptance.log
...
build-static:
	...
	@./vendor/$(SERVICE_BASE_PKG)/scripts/build-static deploy/bin/key
...
docker-image:
	...
	@docker build --rm=false -t gcr.io/elxir-core-infra/key:snapshot deploy
